...
void QtViewer::drawSelection()
{
    ...
    auto node = castTo<sofa::simulation::Node*>(currentSelection);
    if(node){
    ...
    auto object = castTo<sofa::core::objectmodel::BaseObject*>(currentSelection);
    if(object){
        ...
        if(position){
            ...
            if(positions){
                drawTool->drawPoints(positions->getValue(), 2.0, RGBAColor::yellow());
            }else{
